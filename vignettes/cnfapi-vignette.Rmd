...
```
If you want to find the nutrient source information for nutrientSourceId 108, call `get_nutrientsource()`:
```{r}
...
```{r}
get_nutrientname(nutrientNameId = 108)
```
...
rbind.data.frame(get_food(foodCode = 45), get_food(foodCode = 46))
```

Now that we've confirmed that we have two types of cheese, let's get the nutrition information for both and compare it. 

```{r}
#The API doesn't accept multiple IDs at once, so we have to make two queries. 
nutrients_amounts <- get_nutrientamount(foodCode = 45)
nutrient_names <- get_nutrientname()
nutrient_sources <- get_nutrientsource()
nutrient_groups <- get_nutrientgroup()
```
